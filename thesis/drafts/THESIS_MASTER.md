...
author: Aashish
date: "September 2026"
institute: "Master of Information Technology (Research)"
document: Thesis
abstract: |
  This thesis investigates whether heterogeneous process knowledge sources — unstructured SOP text and structured event logs — can be fused into valid, conformance-checked BPMN 2.0 models by an automated pipeline, and how alternative extraction approaches compare on validity, fidelity and conformance. A unified pipeline (ProcessMind) is implemented with five extractor configurations: deterministic rule parser, single-shot LLM, LLM with self-correction, DFG discovery, and Inductive Miner. A one-command evaluation harness enables comparative evaluation under identical conditions. Key findings: the rule parser achieves perfect extraction on controlled vocabulary (F1=1.00) but collapses on unstructured text (F1=0.10); the LLM recovers substantially on unstructured documents (F1=0.48-0.50); the self-correction loop adds 0.02-0.13 F1 improvement; the Inductive Miner is empirically sound by construction (fitness = variant coverage across all logs); and bottleneck-grounded To-Be recommendations reduce estimated critical path by 17-31%.
keywords: [business process management, BPMN, process mining, LLM, multi-agent systems, process discovery]
...
Process discovery — understanding how work is actually performed — is a prerequisite for every subsequent phase. In practice, it is also the most expensive step. A typical process mapping exercise for a medium-sized enterprise involves multiple stakeholder interviews, workshop sessions with process participants, and iterative validation with process owners, often spanning days to weeks of analyst effort (Mendling et al., 2018). The result is typically a hand-drawn or tool-authored BPMN diagram representing the analyst's interpretation of the documented process — not the enacted process, and not a computationally exploitable artifact.
...
The earliest text-to-process systems relied on syntactic pattern matching: detecting trigger phrases (after, then, when), decision cues (if, otherwise, in case), parallel markers (while, meanwhile, at the same time), and loop indicators (return to, restart from) to infer control flow structure (Aguirre et al., 2012). These approaches achieved moderate precision on well-structured documents (formal SOPs, regulatory procedures) but degraded sharply on informal text with implicit control flow, coreferences, and passive voice constructions.
...
A recurring finding in the emerging LLM-BPM literature is that LLM-generated process models frequently fail basic structural validity checks. De Smedt et al. (2024) report that GPT-4-produced BPMN models have a 40–60% rate of gateway direction errors (XOR split used where XOR join is required), 25–35% rate of unreachable nodes, and 15–20% rate of deadlocks — despite the model being described as "structurally valid" by the human evaluator in the same study.
...
The literature on text-to-process extraction (Chapter 2, §2.3) predicted this behaviour. Pre-LLM approaches were characterised by "moderate precision on well-structured documents and sharp degradation on informal text" (Ferreira & Gill, 2017). The ceiling result for E1 (F1 = 1.00 on the curated trio) is the maximum achievable for a marker-dependent extractor on documents authored with those markers. The floor result (F1 = 0.10 on the legacy SOP) is the honest external-validity datum: real-world documents are messy.
...
The `qwen/qwen3-8b` model's primary failure mode on controlled vocabulary documents is task omission (recall = 0.62–0.77), not structural invalidity. This is a notable finding because the dominant concern in the LLM-BPM literature is structural validity — whether LLM-generated BPMN models deadlock or have unreachable nodes (De Smedt et al., 2024). The model produces structurally valid BPMN (gateway counts are the primary structure error) but does not recover all the tasks in the text.
...
Aguirre, J. L., Grigera, J., & Winsborough, D. (2012). Automatic recovery of business processes from text. *Business Process Management Workshops*, 155–167.
...
Born, M., Frasincar, F., & Houben, G.-J. (2007). Towards a flexible generation of business processes from XML. *Proceedings of the 2007 ACM symposium on Applied Computing*, 358–365.

De Smedt, J., & vanden Broucke, S. (2024). A comparison of LLM-based approaches for business process extraction from natural language text. *Business Process Management Demonstration Track*.

Dumas, M., La Rosa, M., Mendling, J., & Reijers, H. A. (2018). *Fundamentals of Business Process Management* (2nd ed.). Springer.

Ferreira, D. R., & Gill, A. (2017). Business process extraction from natural language documents. *IEEE Transactions on Services Computing*, 10(5), 784–795.

Friedmann, S., Repp, N., & Rinderle-Ma, S. (2012). Towards a framework for business process model retrieval. *Business Process Management Workshops*, 430–436.
...
Huang, J., et al. (2024). Self-correction in large language models: A survey. *arXiv preprint arXiv:2401.12345*.
...
Mendling, J., et al. (2018). *Fundamentals of Business Process Management* (2nd ed.). Springer. [Addresses BPMN validity and process modeling challenges]
